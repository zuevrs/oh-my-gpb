...
Before you start:
1. Read `.oma/packs/oh-my-akitagpb/templates/scan/state-contract.json` and treat it as the canonical scan persistence contract.
...
4. Read `.oma/packs/oh-my-akitagpb/runtime/shared/data-handling-policy.json`.
5. Read the installed rule files under `.oma/packs/oh-my-akitagpb/instructions/rules/`.
...
- if OpenAPI or AsyncAPI is absent, do not crash or invent one; continue honestly from code and repo evidence and record the available contract evidence in `.oma/packs/oh-my-akitagpb/state/shared/scan/scan-state.json`
