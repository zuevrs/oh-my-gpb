...
2. Read `.oma/templates/write/state-contract.json`, `.oma/state/shared/write/generated-artifacts.json`, and `.oma/state/shared/write/provenance-bundle.json` from disk before evaluating any artifact.
...
Then:
- validate only the reviewed artifact set recorded in `.oma/state/shared/write/generated-artifacts.json` against `.oma/state/shared/write/provenance-bundle.json` and manifest-listed bundle truth
...
- if the generated artifact set diverges from `.oma/state/shared/write/provenance-bundle.json`, fail validation with a clear `lineage-drift` finding instead of treating it as a warning
...
If validation passes, report the validated artifact bundle and name the runtime proof command if one exists. If validation fails or blocks, stop with the exact reason.
